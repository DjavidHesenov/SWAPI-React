...
export default function VehicleDetails() {
  ...
  const allIds = (state.data.vehicles || [])
    .map(v => parseInt(v.url.split('/').filter(Boolean).pop() || '0'))
    .filter(n => !isNaN(n));
  const sortedIds = allIds.sort((a, b) => a - b);
  const index = sortedIds.indexOf(vehicleId);
  ...
  return (
    <div className="container mx-auto px-6 py-12 text-white">
      ...
      <div className="flex justify-center gap-10 items-center text-yellow-400 text-lg p-8">
        <button
          onClick={() => navigate(`/vehicles/${prevId}`)}
        ...
        <button
          onClick={() => navigate(`/vehicles/${nextId}`)}
